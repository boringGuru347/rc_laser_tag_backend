...
export default class Packet extends Uint8Array {
  ...
  /**
   * ASCII string of the Packet (interprets bytes as ASCII characters).
   * Non-printable characters (< 0x20 or > 0x7E) are replaced with '.'
   * @example
   * console.log(Packet.fromHex('48656C6C6F').ascii)
   * // Hello
   * @member {string}
   */
  get ascii () {
    return _.map(this, byte => {
  ...
  /**
   * Attempts to decode the Packet as a UTF-8 string, falling back to ASCII if decoding fails.
   * @example
   * console.log(Packet.fromHex('48656C6C6F').text)
   * // Hello
   * @member {string}
   */
  get text () {
