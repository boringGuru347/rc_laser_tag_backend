...
## 🔄 Converting Hex Data to Strings

When working with PN532, data is transmitted as hexadecimal bytes. This library provides multiple convenient ways to convert hex data into readable strings.

### Quick Example

```js
// After receiving data from PN532
const response = await pn532.inDataExchange({ tg: 1, data: myCommand })

// Convert to string - multiple options:
console.log(response.data.hex)     // "48656C6C6F" (hex representation)
console.log(response.data.utf8)    // "Hello" (UTF-8 decoded)
console.log(response.data.ascii)   // "Hello" (ASCII, safe for binary)
console.log(response.data.text)    // "Hello" (auto-detect, recommended ✅)

// Or use helper methods:
console.log(response.getUtf8())    // "Hello"
console.log(response.getText())    // "Hello" (recommended ✅)
```
...
|--------|-------------|----------|
| `.hex` | Uppercase hex string | Debugging, logging raw data |
| `.utf8` | UTF-8 decoded string | Known UTF-8 text data |
| `.ascii` | ASCII (non-printable as dots) | Mixed binary/text data |
| `.text` | Auto-detect (UTF-8 → ASCII) | **General purpose** ✅ |

### Sending Strings to PN532

```js
// Convert string to hex for sending
const message = 'Hello NFC'
const packet = Packet.fromUtf8(message)

console.log(packet.hex)  // "48656C6C6F204E4643"

await pn532.inDataExchange({ tg: 1, data: packet })
```

### Using Utility Functions

```js
import { hexToUtf8, hexToAscii, hexToText } from 'pn532.js/utils'

const hex = '48656C6C6F'
console.log(hexToUtf8(hex))   // "Hello"
console.log(hexToAscii(hex))  // "Hello"
console.log(hexToText(hex))   // "Hello" (recommended ✅)
```

### 📚 Full Documentation

- **[Quick Reference](./HEX_TO_STRING_QUICKREF.md)** - One-page cheat sheet
- **[Complete Guide](./HEX_TO_STRING_GUIDE.md)** - Detailed documentation with examples
- **[Example Code](./examples/hex-to-string-conversion.js)** - Working examples you can run

Run the example:
```bash
node examples/hex-to-string-conversion.js
```
